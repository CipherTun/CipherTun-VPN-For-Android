...
open class RoomPreferenceDataStore(createDao: () -> KeyValueEntity.Dao) : PreferenceDataStore() {
    ...
    init {
        CoroutineScope(SupervisorJob() + Dispatchers.IO).launch {
            for (write in pendingWrites) {
                try { write() } catch (exception: Exception) { Log.e("RoomPreferenceDataStore", "write preferences", exception) }
            }
        }
    }

    suspend fun initialize() = withContext(Dispatchers.IO) { cache.size; Unit }

    suspend fun flush() {
        val completion = CompletableDeferred<Unit>()
        synchronized(writeAccess) { check(pendingWrites.trySend { completion.complete(Unit) }.isSuccess) }
        completion.await()
    }
    ...
    override fun putString(key: String, value: String?) = if (value == null) remove(key) else put(KeyValueEntity(key).put(value))
    override fun putStringSet(key: String, values: MutableSet<String>?) = if (values == null) remove(key) else put(KeyValueEntity(key).put(values))
    ...
    fun registerChangeListener(listener: OnPreferenceDataStoreChangeListener) { synchronized(listeners) { listeners.add(listener) } }
    fun unregisterChangeListener(listener: OnPreferenceDataStoreChangeListener) { synchronized(listeners) { listeners.remove(listener) } }
}
